 ...
 * to the app-api origin are touched, and a caller that sets the header itself
 * (e.g. `workspace` for app-less shell widgets) wins.
 */
...
export function installAppContextHeader(): void {
  ...
  window.fetch = (input: RequestInfo | URL, init?: RequestInit) => {
    if (!requestUrl(input).startsWith(apiBase)) return originalFetch(input, init);
