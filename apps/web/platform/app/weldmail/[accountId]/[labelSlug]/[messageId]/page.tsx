...
function mapMessage(msg: Record<string, unknown>, labelSlug: string, noSubject: string, unknown: string): EmailMessage {
  const from = msg.from as { email?: string; name?: string } | string | null | undefined;
  const fromEmail = typeof from === 'object' && from !== null ? from.email : (from as string | undefined);
  const fromName =
...
export default function LabelMessagePage() {
  const { t } = useI18n();
  ...
  const isDraft = messageId.startsWith('draft_') || messageId.startsWith('draft-');

  // Redirect drafts to compose page
  useEffect(() => {
    if (messageId.startsWith('draft_')) {
      router.replace(`/weldmail/${accountId}/${labelSlug}/compose?draftId=${messageId}`);
    }
  }, [messageId, accountId, labelSlug, router]);

  // Refetch message detail when mail:refresh fires (e.g. contact renamed)
  useEffect(() => {
    const handler = () => {
  ...
  const { data: messageData, isLoading: messageLoading } = useMailMessage(messageId, !isDraft);
  ...
  const { data: threadData } = useMailThread(accountId, messageId, !!msg && !isDraft);
  const { data: labelsData } = useMailLabels(accountId, !isDraft);
  const { data: draftsData } = useMailDrafts(accountId, !isDraft);

  // --- Derive component-ready values ---

  const selectedMessage = useMemo<EmailMessage | null>(
    () => (msg ? mapMessage(msg as unknown as Record<string, unknown>, labelSlug, t.mail.shared.noSubject, t.mail.shared.unknown) : null),
  ...
  const availableLabels = useMemo<Mail.Label[]>(() => {
    return (labelsData?.data as Mail.Label[] | undefined) ?? [];
  }, [labelsData]);
    ...
    if (selectedMessage && messageId !== lastRefreshedId.current) {
      ...
      if (hasUnread && threadId) {
        markThreadRead.mutate(
          { accountId, threadId, isRead: true },
  ...
  }, [selectedMessage, messageId, threadMessages, threadId, accountId, markThreadRead]);
