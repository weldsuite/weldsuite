 ...
 * `MeetingRoomView` the WeldMeet experience uses, so an ad-hoc channel / DM
 * call renders the exact polished meeting room.
 *
 * This is the WeldChat analogue of `app/weldmeet/components/meeting-overlay.tsx`
 * (`MeetingRoomAdapter`). The two call contexts are twins — same Cloudflare
 * RealtimeKit client, same `ViewMode` union — so the mapping is almost 1:1.
 * Chat-call chrome is intentionally LEAN: channel chat tab only, no host
 * controls, recording, or share-link.
 */
...
function ChatMeetingRoomAdapter() {
  ...
  useEffect(() => {
    ...
    const updateParticipants = () => {
      const joined = (meeting.participants?.joined?.toArray?.() ?? []) as RTKParticipant[];
      setParticipants([meeting.self, ...joined]);
    };

    const sharingPeers = () =>
      new Set<string>(
        ((meeting.participants?.joined?.toArray?.() ?? []) as RTKParticipant[])
  ...
  const ringingPlaceholders = useMemo((): MeetingPeer[] => {
    if (!isDmOrGroup || ringingExpired) return [];
    const joinedKeys = new Set(
      participants.slice(1).flatMap((p) => [p?.userId, p?.customParticipantId].filter(Boolean) as string[]),
    );
    return otherMembers
  ...
  const allParticipants = useMemo(
    (): MeetingPeer[] => [...(participantsWithSelfScreenShare as MeetingPeer[]), ...ringingPlaceholders],
