...
interface AiActiveListClientProps {
  initialConversations: Helpdesk.Conversation[];
}

export default function AiActiveListClient({ initialConversations }: Readonly<AiActiveListClientProps>) {
  ...
  const handleItemClick = async (item: ConversationItem) => {
    ...
    router.push(`/welddesk/ai-active/${item.id}`);
  };
      ...
      <PopoverContent align="start" className="w-[220px] p-3">
        ...
        <div className="flex flex-wrap gap-1.5">
          {([
            ...
            { value: 'active' as FilterType, label: ta.active },
            { value: 'pending' as FilterType, label: ta.pending },
          ]).map(({ value, label }) => (
            <Toggle
              key={value}
              size="sm"
  ...
  return (
    <ConversationList
      ...
      getItemUrl={(item) => `/welddesk/ai-active/${item.id}`}
      onItemClick={handleItemClick}
      filterContent={filterContent}
      onToggleStar={handleToggleStar}
      emptyMessage={ta.noActiveAiConversations}
    />
  );
}
