...
export function NewsEditorClient({ newsId }: NewsEditorClientProps) {
  ...
  const [excerpt, setExcerpt] = useState('We are thrilled to announce that WeldSuite has reached a major milestone with 10,000 active users worldwide.');
      ...
      <div className="flex">
        ...
        {coverImage ? (
          <div className="relative mb-12 -mx-24 group">
            <img src={coverImage} alt={st('sweep.welddesk.newsEditor.coverImageAlt')} className="w-full h-[40vh] object-cover rounded-lg" />
        ...
        <div className="relative">
          <div
            ...
            data-placeholder={st('sweep.welddesk.newsEditor.contentPlaceholder')}
            style={{
              caretColor: 'currentColor',
              direction: 'ltr',
              unicodeBidi: 'normal',
        ...
        <div className="w-80 border-l border-gray-200 bg-white text-gray-900 flex flex-col">
          ...
          <div className="p-4 border-b border-gray-200">
            <div className="flex items-center justify-between mb-3">
              <h3 className="font-semibold text-sm">{tn.articleSettings}</h3>
            </div>
          </div>
          ...
          <ScrollArea className="flex-1">
            <div className="p-4 space-y-4">
              ...
              <div className="space-y-2">
                <Label className="text-xs">{tn.author}</Label>
                <Input
                  value={author}
                  onChange={(e) => setAuthor(e.target.value)}
                  placeholder={tn.enterAuthorName}
                  className="text-sm"
                />
              </div>
              ...
              <div className="flex items-center justify-between gap-4">
                <Label className="text-xs">{tn.date}</Label>
                <Popover open={isCalendarOpen} onOpenChange={setIsCalendarOpen}>
                  <PopoverTrigger asChild>
                    <div className="relative w-[180px]">
                      <Input
                        value={dateInputValue}
                        placeholder={st('sweep.welddesk.newsEditor.dateInputPlaceholder')}
                      ...
                      >
                        ...
                        <span className="sr-only">{tn.selectDate}</span>
                      </Button>
                    </div>
                  </PopoverTrigger>
                  <PopoverContent
              ...
              {/* Category */}
              <div className="flex items-center justify-between gap-4">
                <Label className="text-xs">{tn.category}</Label>
              ...
              {/* Status */}
              <div className="flex items-center justify-between gap-4">
                <Label className="text-xs">{tn.status}</Label>
                <Select value={status} onValueChange={setStatus}>
                  <SelectTrigger className="text-sm w-[180px]">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="draft">{tn.draft}</SelectItem>
                    <SelectItem value="published">{tn.published}</SelectItem>
                    <SelectItem value="scheduled">{tn.scheduled}</SelectItem>
                  </SelectContent>
                </Select>
              </div>
          ...
          <div className="bg-background border border-border rounded-lg shadow-lg w-full max-w-md mx-4 animate-in zoom-in-95">
            <div className="p-6 space-y-4">
              <div className="flex items-center justify-between">
                <h3 className="text-lg font-semibold">{tn.insertLink}</h3>
                <Button
                  variant="ghost"
                  size="icon"
                  className="h-8 w-8"
              ...
              <div className="space-y-4">
                <div className="space-y-2">
                  <label className="text-sm font-medium">{tn.linkText}</label>
                  <Input
                    value={linkText}
                    onChange={(e) => setLinkText(e.target.value)}
                    placeholder={tn.enterLinkText}
                    onKeyDown={(e) => {
                      if (e.key === 'Enter') {
                        e.preventDefault();
                        insertLink();
                ...
                <div className="space-y-2">
                  <label className="text-sm font-medium">{tn.url}</label>
                  <Input
                    ref={linkInputRef}
                    value={linkUrl}
                    onChange={(e) => setLinkUrl(e.target.value)}
                    placeholder={st('sweep.welddesk.newsEditor.urlPlaceholder')}
                    type="url"
                    autoFocus
                    onKeyDown={(e) => {
                      if (e.key === 'Enter') {
