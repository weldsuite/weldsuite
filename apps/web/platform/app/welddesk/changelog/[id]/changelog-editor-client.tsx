...
interface ChangelogEditorClientProps {
  changelogId: string;
}
...
export function ChangelogEditorClient({ changelogId }: ChangelogEditorClientProps) {
  ...
  useBreadcrumbs([
    ...
    { label: tc.title, href: '/welddesk/changelog' },
    { label: tc.editEntry },
  ]);
  ...
  const handleCoverImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      if (!file.type.startsWith('image/')) {
        alert(tc.selectImageOnly);
        return;
      }

      // Check file size (max 5MB)
      if (file.size > 5 * 1024 * 1024) {
        alert(tc.imageSizeLimit);
        return;
      }
  ...
  const commands: Command[] = [
    {
      id: 'text',
      label: tc.cmdText,
      description: tc.cmdTextDesc,
      icon: <Type className="h-4 w-4" />,
      action: () => insertBlock('p'),
    },
    {
      id: 'heading1',
      label: tc.cmdHeading1,
      description: tc.cmdHeading1Desc,
      icon: <Heading1 className="h-4 w-4" />,
      action: () => insertBlock('h1'),
    },
    {
      id: 'heading2',
      label: tc.cmdHeading2,
      description: tc.cmdHeading2Desc,
      icon: <Heading2 className="h-4 w-4" />,
      action: () => insertBlock('h2'),
    },
    {
      id: 'heading3',
      label: tc.cmdHeading3,
      description: tc.cmdHeading3Desc,
      icon: <Heading3 className="h-4 w-4" />,
      action: () => insertBlock('h3'),
    },
    {
      id: 'bulletlist',
      label: tc.cmdBulletedList,
      description: tc.cmdBulletedListDesc,
      icon: <List className="h-4 w-4" />,
      action: () => insertBlock('ul'),
    },
    {
      id: 'numberedlist',
      label: tc.cmdNumberedList,
      description: tc.cmdNumberedListDesc,
      ...
      <div className="flex">
        ...
        {coverImage ? (
          <div className="relative mb-12 -mx-24 group">
            <img src={coverImage} alt={st('sweep.welddesk.changelogEditor.coverImageAlt')} className="w-full h-[40vh] object-cover rounded-lg" />
        ...
        <div className="relative">
          <div
            ...
            data-placeholder={tc.pressForCommands}
            style={{
              caretColor: 'currentColor',
              direction: 'ltr',
              unicodeBidi: 'normal',
        ...
        <div className="w-80 border-l border-gray-200 bg-white text-gray-900 flex flex-col">
          ...
          <div className="p-4 border-b border-gray-200">
            <div className="flex items-center justify-between mb-3">
              <h3 className="font-semibold text-sm">{tc.changelogSettings}</h3>
            </div>
          </div>
          ...
          <ScrollArea className="flex-1">
            <div className="p-4 space-y-4">
              ...
              <div className="space-y-2">
                <Label className="text-xs">{tc.author}</Label>
                <Input
                  value={author}
                  onChange={(e) => setAuthor(e.target.value)}
                  placeholder={tc.enterAuthorName}
                  className="text-sm"
                />
              </div>
                  ...
                  <PopoverTrigger asChild>
                    <div className="relative w-[180px]">
                      ...
                      >
                        ...
                        <span className="sr-only">{tc.selectDate}</span>
                      </Button>
                    </div>
                  </PopoverTrigger>
                  <PopoverContent
              ...
              {/* Category */}
              <div className="flex items-center justify-between gap-4">
                <Label className="text-xs">{tc.category}</Label>
                <Select value={category} onValueChange={setCategory}>
                  <SelectTrigger className="text-sm w-[180px]">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="company">🏢 {tc.company}</SelectItem>
                    <SelectItem value="product">📦 {tc.product}</SelectItem>
                    <SelectItem value="industry">📊 {tc.industry}</SelectItem>
                    <SelectItem value="announcement">📢 {tc.categoryAnnouncement}</SelectItem>
                  </SelectContent>
                </Select>
              </div>
          ...
          <div className="bg-background border border-border rounded-lg shadow-lg w-full max-w-md mx-4 animate-in zoom-in-95">
            <div className="p-6 space-y-4">
              <div className="flex items-center justify-between">
                <h3 className="text-lg font-semibold">{tc.insertLink}</h3>
                <Button
                  variant="ghost"
                  size="icon"
                  className="h-8 w-8"
              ...
              <div className="space-y-4">
                <div className="space-y-2">
                  <label className="text-sm font-medium">{tc.linkText}</label>
                  <Input
                    value={linkText}
                    onChange={(e) => setLinkText(e.target.value)}
                    placeholder={tc.enterLinkText}
                    onKeyDown={(e) => {
                      if (e.key === 'Enter') {
                        e.preventDefault();
                        insertLink();
                ...
                <div className="space-y-2">
                  <label className="text-sm font-medium">{tc.url}</label>
                  <Input
                    ref={linkInputRef}
                    value={linkUrl}
                    onChange={(e) => setLinkUrl(e.target.value)}
                    placeholder={st('sweep.welddesk.changelogEditor.urlPlaceholder')}
                    type="url"
                    autoFocus
                    onKeyDown={(e) => {
                      if (e.key === 'Enter') {
