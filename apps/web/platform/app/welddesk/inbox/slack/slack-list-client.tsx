...
interface SlackListClientProps {
...
export default function SlackListClient({ initialConversations, accessToken }: SlackListClientProps) {
  ...
  const { isConnected } = useHelpdeskWebSocket({
    ...
    onNewConversation: (newConversation) => {
      if (newConversation.channel !== 'slack') return;
  ...
  const handleItemClick = async (item: ConversationItem) => {
    ...
    router.push(`/welddesk/inbox/slack/${item.id}`);
  };
  ...
  return (
    <ConversationList
      ...
      getItemUrl={(item) => `/welddesk/inbox/slack/${item.id}`}
      onItemClick={handleItemClick}
      filterContent={filterContent}
      onToggleStar={handleToggleStar}
      contextMenuItems={getContextMenu}
      emptyMessage={ti.noSlackMessages}
    />
  );
}
