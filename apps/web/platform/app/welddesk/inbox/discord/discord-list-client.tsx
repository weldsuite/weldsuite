...
function conversationToItem(conv: Helpdesk.Conversation): ConversationItem {
  return {
    id: conv.id,
    name: conv.customerName || conv.customerEmail || 'Unknown User',
...
interface DiscordListClientProps {
...
export default function DiscordListClient({ initialConversations, accessToken }: DiscordListClientProps) {
  ...
  useHelpdeskWebSocket({
    ...
    onNewConversation: (newConversation) => {
      if (newConversation.channel !== 'discord') return;
  ...
  const handleItemClick = async (item: ConversationItem) => {
    ...
    router.push(`/welddesk/inbox/discord/${item.id}`);
  };
  ...
  return (
    <ConversationList
      ...
      getItemUrl={(item) => `/welddesk/inbox/discord/${item.id}`}
      onItemClick={handleItemClick}
      filterContent={filterContent}
      onToggleStar={handleToggleStar}
      contextMenuItems={getContextMenu}
      emptyMessage={ti.noDiscordMessages}
    />
  );
}
