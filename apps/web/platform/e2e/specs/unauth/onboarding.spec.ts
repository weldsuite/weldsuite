 ...
 *   3. `/onboarding`     — workspace name + optional location/apps
...
test.describe('Onboarding — new-user signup + workspace creation', () => {
  ...
  test('full signup → onboarding wizard → dashboard', async ({ page }) => {
    ...
    // ------------------------------------------------------------------
    // 3. One-screen workspace setup: only a name is required.
    // ------------------------------------------------------------------

    await expect(page.getByRole('heading', { name: 'Create your workspace' })).toBeVisible({ timeout: 15_000 });
    ...
    await page.locator('#organizationName').fill('   ');
    await expect(getStartedBtn).toBeDisabled();
    ...
    // ------------------------------------------------------------------
    // 4. Provisioning screen + dashboard redirect
    // ------------------------------------------------------------------
