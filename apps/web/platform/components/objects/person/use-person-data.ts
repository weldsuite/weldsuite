...
function useBulkUpdatePeople() {
  ...
  return useMutation({
    mutationFn: async (input: {
      ...
      >(`/people/bulk-update`, input);
    },
    onSuccess: (_, variables) => {
      const ids = new Set(variables.personIds);
      const patch = variables.updates;
      qc.setQueriesData<ListEnvelope<Person>>({ queryKey: personKeys.lists() }, (prev) => {
        if (!prev?.data) return prev;
        return {
          ...prev,
          data: prev.data.map((p) => (ids.has(p.id) ? ({ ...p, ...patch } as Person) : p)),
        };
      });
      qc.setQueriesData<{ pages: ListEnvelope<Person>[]; pageParams: unknown[] }>(
        { queryKey: [...personKeys.lists(), 'infinite'] },
        (prev) => {
          ...
          return {
            ...
            pages: prev.pages.map((page) => ({
              ...page,
              data: page.data.map((p) => (ids.has(p.id) ? ({ ...p, ...patch } as Person) : p)),
            })),
          };
        },
      );
      for (const id of variables.personIds) {
        qc.invalidateQueries({ queryKey: personKeys.detail(id) });
        qc.invalidateQueries({ queryKey: personKeys.detailFull(id) });
      }
    },
    onError: (err) => {
      console.error('[People] bulk-update failed:', err);
