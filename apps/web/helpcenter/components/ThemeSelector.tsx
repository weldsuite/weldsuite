...
export function ThemeSelector(props: Readonly<React.ComponentPropsWithoutRef<typeof Listbox<'div'>>>) {
  ...
  return (
    <Listbox as="div" value={theme} onChange={setTheme} {...props}>
      ...
      <ListboxButton className="flex h-6 w-6 items-center justify-center rounded-lg shadow-md ring-1 shadow-black/5 ring-black/5 dark:bg-slate-700 dark:ring-white/5 dark:ring-inset" aria-label="Theme">
        <LightIcon className={clsx('h-4 w-4 dark:hidden', theme === 'system' ? 'fill-slate-400' : 'fill-sky-400')} />
        <DarkIcon className={clsx('hidden h-4 w-4 dark:block', theme === 'system' ? 'fill-slate-400' : 'fill-sky-400')} />
      </ListboxButton>
