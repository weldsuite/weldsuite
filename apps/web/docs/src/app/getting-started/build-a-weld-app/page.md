...
Build a small web app that WeldSuite hosts and opens at `/apps/{code}`. {% .lead %}

WeldApps are separate Vite frontends. The platform loads them in a sandboxed iframe and gives them a scoped token for the **WeldSuite App API** (`https://api.weldsuite.org/v1`). They do not receive a Clerk session or first-party `/api/*` access.
...
1. Install the CLI (`Node 20+`):
   ...
   export WELD_API_KEY=wsk_...   # Settings → API keys
   ```
...
3. Install (or re-open) the app — the sidenav routes to `/apps/{code}` and loads the R2 bundle.

Optional: `weld app publish` submits a public listing. First-party publisher workspaces skip review and show an **Official** badge.
...
- CLI reference lives with [`@weldsuite/cli`](https://www.npmjs.com/package/@weldsuite/cli)
