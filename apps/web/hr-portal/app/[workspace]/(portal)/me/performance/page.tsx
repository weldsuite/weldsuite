          ...
          <div className="grid gap-3 sm:grid-cols-2">
            {data.kpis.map((kpi) => (
              <div key={kpi.id} className="rounded-md border border-gray-100 p-3">
                ...
                <p className="text-xs text-gray-500 mt-1">{format(dict.performance.period, { start: formatDate(kpi.periodStart, locale), end: formatDate(kpi.periodEnd, locale) })}</p>
            ...
            {data.milestones.map((m) => (
              <li key={m.id} className="py-3">
                ...
                <p className="text-xs text-gray-400 mt-1">
                  {m.achievedAt
                    ? format(dict.client.milestones.achievedOn, { date: formatDate(m.achievedAt, locale) })
                    : m.dueDate
                      ? format(dict.client.milestones.due, { date: formatDate(m.dueDate, locale) })
                      : dict.common.none}
                </p>
              </li>
            ))}
