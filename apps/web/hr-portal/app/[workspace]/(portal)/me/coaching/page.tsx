          ...
          {data.map((log) => (
            <Card key={log.id}>
              <div className="flex flex-wrap items-start justify-between gap-2">
                <div>
                  <p className="text-sm text-gray-500">{formatDate(log.sessionDate, locale)}</p>
              ...
              {log.followUpDate && (
                <p className="text-xs text-gray-500 mt-2">
                  {dict.coaching.followUp}: {formatDate(log.followUpDate, locale)}
                </p>
              )}

              {log.acknowledgedAt ? (
                <p className="text-xs text-gray-400 mt-3">{format(dict.coaching.acknowledgedOn, { date: formatDate(log.acknowledgedAt, locale) })}</p>
              ) : openComment === log.id ? (
