...
export default function ClientTeamMemberPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        ...
        <p className="text-xs text-gray-400 mt-1">{format(dict.client.teamMember.since, { date: formatDate(member.assignedSince, locale) })}</p>
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                {kpis.map((kpi) => (
                  <div key={kpi.id} className="rounded-md border border-gray-100 p-3">
                    ...
                    <p className="text-xs text-gray-500">{format(dict.performance.period, { start: formatDate(kpi.periodStart, locale), end: formatDate(kpi.periodEnd, locale) })}</p>
                ...
                {evaluations.map((ev) => (
                  <li key={ev.id} className="py-3">
                    ...
                    {ev.periodStart && ev.periodEnd && (
                      <p className="text-xs text-gray-500">{format(dict.performance.period, { start: formatDate(ev.periodStart, locale), end: formatDate(ev.periodEnd, locale) })}</p>
                    )}
