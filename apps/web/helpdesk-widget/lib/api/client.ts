  ...
  async getWidgetSettings(widgetId?: string): Promise<ApiResponse<WidgetSettings>> {
    ...
    const settings: WidgetSettings = {
      ...
      themeSettings: {
        ...
        borderRadius: Number.parseInt(config.styling?.borderRadius) || 20,
        fontSize: Number.parseInt(config.styling?.fontSize) || 14,
  ...
  async startConversation(data: StartConversationRequest): Promise<StartConversationResponse> {
    ...
    return {
      ...
      data: result.conversation ? {
        ...
        conversationNumber: Number.parseInt(result.conversation.conversationNumber?.replace('CONV-', '') || '0'),
