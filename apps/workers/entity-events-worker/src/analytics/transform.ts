/**
 * Entity event -> analytics row.
 *
 * Moved verbatim from analytics-worker/src/services/event-processor.ts when the
 * analytics sink became a dispatcher consumer. The ENTITY_CONFIG map below is
 * hand-maintained: an entity type absent from it still produces a row, just
 * with only the auto-extracted common fields.
 */
