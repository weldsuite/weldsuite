...
function mapProduct(record: Record<string, unknown>, externalId: string): MappedProduct | null {
  ...
  if (!name) return null;
  const slug = pickString(record, ['slug'], 255) ?? slugify(name);
  ...
  const price = pickString(record, ['price', 'regular_price']) ?? '0';
  ...
  return {
    ...
    values: compact({
      name,
      slug,
      sku: pickString(record, ['sku'], 100),
      description: pickString(record, ['description']),
      shortDescription: pickString(record, ['short_description'], 500),
      price,
      compareAtPrice: pickString(record, ['regular_price']) !== price ? pickString(record, ['regular_price']) : null,
      currency: pickString(record, ['currency'], 3) ?? 'EUR',
      status,
      featuredImageUrl: images[0]?.url ?? pickString(record, ['images.0.src'], 500),
      ...
      trackInventory: record.manage_stock === true,
      productType: pickString(record, ['type'], 100),
      publishedAt: pickString(record, ['date_created_gmt', 'date_created'])
        ? new Date(pickString(record, ['date_created_gmt', 'date_created'])!)
        : null,
    }),
  };
}
...
function mapOrder(record: Record<string, unknown>, externalId: string): MappedOrder | null {
  const orderNumber = pickString(record, ['number', 'id'], 50);
  if (!orderNumber) return null;
  const wcStatus = pickString(record, ['status']) ?? 'pending';
  const billing = (record.billing as Record<string, unknown> | undefined) ?? undefined;
  const shipping = (record.shipping as Record<string, unknown> | undefined) ?? undefined;
  const customerName =
  ...
  const paidAt = pickString(record, ['date_paid_gmt', 'date_paid']);
  ...
  return {
    ...
    lineItems: lineItemsRaw.map((item) => ({
      ...
      name: pickString(item, ['name'], 255) ?? 'Item',
    ...
    values: compact({
      ...
      source: 'woocommerce',
      customerEmail: pickString(billing ?? record, ['email', 'billing.email'], 255),
      customerName: customerName ? customerName.slice(0, 255) : null,
      customerPhone: pickString(billing ?? {}, ['phone'], 50),
      status: ORDER_STATUS[wcStatus] ?? 'pending',
      paymentStatus: paidAt || PAID_STATUSES.has(wcStatus) ? 'paid' : 'pending',
      currency: pickString(record, ['currency'], 3) ?? 'EUR',
      subtotal: pickString(record, ['subtotal']) ?? '0',
      discountTotal: pickString(record, ['discount_total']) ?? '0',
      shippingTotal: pickString(record, ['shipping_total']) ?? '0',
      ...
      paymentMethod: pickString(record, ['payment_method_title', 'payment_method'], 100),
      paidAt: paidAt ? new Date(paidAt) : null,
      customerNote: pickString(record, ['customer_note']),
      itemCount: lineItemsRaw.length,
      totalQuantity: lineItemsRaw.reduce((sum, item) => sum + (pickNumber(item, ['quantity']) ?? 0), 0),
      completedAt: wcStatus === 'completed' && pickString(record, ['date_completed_gmt', 'date_modified_gmt'])
        ? new Date(pickString(record, ['date_completed_gmt', 'date_modified_gmt'])!)
        : null,
    }),
  };
}
...
function mapPerson(record: Record<string, unknown>, externalId: string): MappedPerson | null {
  ...
  const billing = (record.billing as Record<string, unknown> | undefined) ?? record;
...
export function modifiedAtOf(record: Record<string, unknown>): string | null {
  return pickString(record, ['date_modified_gmt', 'date_modified', 'date_created_gmt']);
}

export function mapConnectorRecord(entity: ConnectorEntity, record: Record<string, unknown>): MappedRecord | null {
  ...
  switch (entity) {
    ...
    case 'order':
      return mapOrder(record, externalId);
    case 'person':
