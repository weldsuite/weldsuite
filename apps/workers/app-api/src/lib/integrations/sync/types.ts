 ...
 *
 * Ported verbatim from api-worker (`src/lib/integrations/sync/types.ts`) as part
 * of W5b. Only the `OAuthTokens` import is adapted: api-worker kept a local
 * duplicate in `../types`, but it is byte-identical to the canonical one already
 * exported from `@weldsuite/db/schema`, so app-api sources it there instead of
 * re-introducing a second copy.
 *
 * Only `calendar_event` has an adapter here today (google-calendar). The rest of
 * the surface is kept so the adapter can satisfy `CrmSyncAdapter` unchanged.
 */
