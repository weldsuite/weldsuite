/**
 * Cron matching + next-run computation (5-field, timezone-aware, pure).
 *
 * Copy of `apps/workers/workflow-worker/src/lib/cron.ts` — keep the two in sync.
 * Used by WeldAgent cron routines, which the hourly app-api sweep picks up once
 * `next_run_at` is due.
 */
