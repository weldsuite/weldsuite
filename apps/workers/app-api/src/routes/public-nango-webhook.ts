 ...
 *
 * The handler always answers 200 on a *recognised* webhook, even when ingest
 * fails: the failure is recorded on the run row and surfaced in the UI, and a
 * Nango retry would replay the same failure. Signature failures answer 401.
 */
...
app.post('/webhook', async (c) => {
  ...
  try {
    if (isAuthWebhook(payload)) return c.json(await handleAuthWebhook(c.env, payload));
    if (isSyncWebhook(payload)) return c.json(await handleSyncWebhook(c.env, payload));
 ...
 */
async function handleAuthWebhook(env: Env, payload: NangoAuthWebhook) {
  const clerkOrgId =
  ...
  if (!clerkOrgId) {
    ...
    return { ok: true, ignored: 'unknown_workspace' };
  }
  ...
  if (!connection) {
    ...
    return { ok: true, ignored: 'unknown_connection' };
  }
  ...
  return { ok: true, status: 'active', operation: payload.operation };
}
...
async function handleSyncWebhook(env: Env, payload: NangoSyncWebhook) {
  const clerkOrgId = await resolveConnectionWorkspace(env, payload.providerConfigKey, payload.connectionId);
  if (!clerkOrgId) {
    console.warn(
      `[nango-webhook] sync webhook for unmapped connection ${payload.providerConfigKey}/${payload.connectionId}`,
    );
    return { ok: true, ignored: 'unknown_workspace' };
  }
  ...
  if (!client) {
    ...
    return { ok: true, ignored: 'nango_not_configured' };
  }
  ...
  try {
    ...
    await finishSyncRun({
      ...
      errorSamples,
      // Only advance the watermark on a complete pass; advancing after a
      // truncated run would silently skip the records we never read.
      watermark: truncated ? null : { model: payload.model, at: runStartedAt },
    });

    return { ok: true, status, applied: totals, truncated };
  } catch (err) {
