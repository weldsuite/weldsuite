 ...
 *
 * One instance per helpdesk conversation. Handles bidirectional
 * messaging between agents and customers, plus typing indicators,
 * presence tracking, system events, and AI token streaming.
 *
...
export class ConversationRoom extends DurableObject<Env> {
  ...
  private handleWebSocketUpgrade(request: Request): Response {
    ...
    const role = request.headers.get(AUTH_HEADERS.ROLE) || 'agent';
    ...
    // Send connected message with current presence
    server.send(
      JSON.stringify({
        type: 'connected',
