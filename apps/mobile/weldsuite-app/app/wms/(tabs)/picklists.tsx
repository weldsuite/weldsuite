...
import { Search, ClipboardList, User, Package, AlertCircle } from 'lucide-react-native';
...
export default function PickListsTabScreen() {
  ...
  const filteredPickLists = (pickLists || []).filter((pickList) => {
    ...
    return (
      ...
      pickList.assignedToName?.toLowerCase().includes(query) ||
          ...
          <View style={styles.pickListHeaderLeft}>
            ...
            {item.assignedToName && (
              <View style={styles.assignedTo}>
                ...
                <Text style={[styles.assignedToText, { color: colors.muted }]}>{item.assignedToName}</Text>
              </View>
            )}
          </View>
  ...
  if (loading && (!pickLists || pickLists.length === 0)) {
    return (
