...
export default function MyTasksScreen() {
  ...
  const loadTasks = async () => {
    try {
      ...
      if (response.success && response.data) {
        ...
        // Map the API response to include completed flag
        const mappedTasks = items.map((task: any) => ({
          ...task,
          completed: task.status === 'done',
        }));
        setTasks(mappedTasks);
        setTotal(response.data.meta?.total || items.length);
      }
    } catch (error) {
            ...
            <View style={styles.taskMeta}>
              ...
              {task.projectName && (
                <View style={styles.projectBadge}>
                  <FolderKanban size={11} color={colors.muted} strokeWidth={2} />
                  <Text style={[styles.projectText, { color: colors.muted }]}>
                    {task.projectName}
                  </Text>
                </View>
              )}
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background, paddingTop: 15 }]}>
      {/* Header */}
      <View style={[styles.header, { backgroundColor: colors.background, borderBottomColor: colors.divider }]}>
        <Text style={[styles.headerTitle, { color: colors.text }]}>My Tasks ({filteredTasks.length})</Text>
      </View>
          ...
          {selectedTask && (
            <ScrollView style={styles.modalContent} showsVerticalScrollIndicator={false}>
              ...
              {selectedTask.projectName && (
                <View style={styles.modalSection}>
                  <Text style={[styles.modalLabel, { color: colors.muted }]}>Project</Text>
                  <View style={styles.modalRow}>
                    <FolderKanban size={16} color={colors.text} strokeWidth={2} />
                    <Text style={[styles.modalText, { color: colors.text }]}>
                      {selectedTask.projectName}
                    </Text>
                  </View>
      ...
      >
        ...
        <SafeAreaView style={[styles.modalContainer, { backgroundColor: colors.background }]}>
          ...
          <ScrollView style={styles.modalContent} showsVerticalScrollIndicator={false}>
            ...
            {/* Priority */}
            <View style={styles.modalSection}>
              <Text style={[styles.modalLabel, { color: '#374151' }]}>Priority</Text>
              <View style={styles.statusOptions}>
                  ...
                  return (
                    <TouchableOpacity
                      ...
                      style={[
                        styles.statusOption,
                        {
                          backgroundColor: isSelected ? priorityConfig?.bgColor : colors.background,
                          borderColor: isSelected ? priorityConfig?.color : '#E5E7EB',
                        }
                      ]}
                    ...
                    >
                      <Text
                        style={[
                          styles.statusOptionText,
                          { color: isSelected ? priorityConfig?.color : colors.text }
                        ]}
                      >
                        {priorityConfig?.label}
                      </Text>
