...
export default function SettingsScreen() {
  ...
  useEffect(() => {
    ...
    Updates.readLogEntriesAsync(24 * 60 * 60 * 1000)
      .then((entries) => {
        if (entries.length === 0) {
          setUpdateLogs('No update logs in the last 24 hours.');
          return;
        }
        setUpdateLogs(
          entries
      ...
      .catch((err) => {
        setUpdateLogs(`Could not read update logs: ${String(err)}`);
      });
  }, []);
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: bg }]}>
      ...
      {updateLogs ? (
        <View style={[styles.section, { backgroundColor: cardBg, borderColor: sectionBorderColor }]}>
          <Text style={[styles.sectionTitle, { color: mutedText }]}>Update logs</Text>
          <Text style={[styles.logText, { color: mutedText }]} selectable>
            {updateLogs}
          </Text>
        </View>
      ) : null}
...
const styles = StyleSheet.create({
  ...
  logText: { fontSize: 11, lineHeight: 16, paddingHorizontal: 18, paddingBottom: 16, fontFamily: 'Menlo' },
});
