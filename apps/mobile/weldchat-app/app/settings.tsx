...
export default function SettingsScreen() {
  ...
  const bg = isDark ? colors.bgPrimary : '#FFFFFF';
  ...
  const handleSwitchWorkspace = async (orgId: string) => {
    if (switchingId || !setActive || orgId === organization?.id) return;
      ...
      <ScrollView style={[styles.container, { backgroundColor: bg }]}>
        ...
        <View style={[styles.section, { backgroundColor: cardBg, borderColor: sectionBorderColor }]}>
          ...
          <View style={[styles.row, { borderBottomColor: borderColor }]}>
            ...
            <Text style={[styles.value, { color: mutedText }]}>
              {user?.emailAddresses[0]?.emailAddress}
            </Text>
          </View>
          {organization && (
            <View style={[styles.row, { borderBottomWidth: 0 }]}>
              <Text style={[styles.label, { color: colors.textPrimary }]}>Workspace</Text>
              <Text style={[styles.value, { color: mutedText }]}>{organization.name}</Text>
            </View>
          )}
        </View>
            ...
            {memberships.map((membership, i) => {
              ...
              return (
                <TouchableOpacity
                  ...
                  onPress={() => !isActive && handleSwitchWorkspace(org.id)}
                ...
                >
                  <View style={styles.rowLeft}>
                    ...
                    <Text style={[styles.label, { color: colors.textPrimary }, isActive && { fontWeight: '600' }]}>{org.name}</Text>
                  </View>
                  {isSwitching ? (
                    <ActivityIndicator size="small" color="#4D94F8" />
                  ) : isActive ? (
          ...
          <TouchableOpacity style={[styles.row, { borderBottomWidth: 0 }]} onPress={toggleTheme} activeOpacity={0.6}>
            <View style={styles.rowLeft}>
              <View style={[styles.iconCircle, { backgroundColor: iconCircleBg }]}>
                {isDark ? <Moon size={18} color={subtleIcon} /> : <Sun size={18} color={subtleIcon} />}
              </View>
        ...
        {/* Sign Out */}
        <TouchableOpacity style={[styles.signOutButton, { borderColor: sectionBorderColor }]} onPress={() => signOut()} activeOpacity={0.6}>
