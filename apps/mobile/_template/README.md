...
- `{{APP_NAME}}`, display name (e.g. `WeldFoo`)
- `{{APP_SLUG}}`, workspace + folder name (e.g. `weldfoo-app`)
- `{{APP_CODE}}`, lowercase one-word id, used for scheme, bundle suffix, notification `APP_CODE` (e.g. `weldfoo`)
- `{{APP_SUBTITLE}}`, tagline shown on the login screen
- `{{BUNDLE_ID}}`, iOS bundle id / Android package (e.g. `com.weldsuite.weldfoo`)
- `{{PRIMARY_COLOR}}`, hex color for brand accents, splash, notification icon
...
- `services/app-api.ts` targeting the unified app-api via `@weldsuite/app-api-client` domain clients (workspaces, dashboard, me, push tokens, add your own domains)
