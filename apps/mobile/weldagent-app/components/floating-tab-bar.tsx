 ...
 *
 * Matches the inset, rounded navigation from WeldBooks: a translucent bar that
 * sits above the home indicator with an active-state highlight.
 */
...
const CENTER_ROUTE = 'new-placeholder';
...
export function FloatingTabBar({ state, descriptors, navigation }: FloatingTabBarProps) {
  ...
  const shellBorder = theme === 'dark' ? 'rgba(255, 255, 255, 0.08)' : 'rgba(0, 0, 0, 0.06)';
  ...
  return (
    ...
    >
      <View style={[styles.shell, { backgroundColor: shellBackground, borderColor: shellBorder }]}>
        <Animated.View
          pointerEvents="none"
          style={[styles.indicator, { backgroundColor: indicatorBackground }, indicatorStyle]}
        />
          ...
          return (
            ...
            >
              {options.tabBarIcon?.({ focused: isFocused, color, size: iconSize }) ??
                (isCenter ? <Sparkles size={iconSize} color={color} strokeWidth={2.2} /> : null)}
            </Pressable>
          );
        })}
      </View>
