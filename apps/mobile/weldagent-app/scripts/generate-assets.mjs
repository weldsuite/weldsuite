/**
 * Rasterize the WeldAgent mark into the Expo asset set.
 *
 * Source of truth is the platform SVG
 * (apps/web/platform/public/assets/images/weldagent/icon.svg) so the mobile
...
if (!OUT_ARG) throw new Error('usage: node generate-assets.mjs <outDir>');
...
function markSvg(size, fill, markRatio, paths) {
  ...
  const x = (size - w) / 2;
  const y = (size - h) / 2;
  const inner = paths.map((d) => `<path fill="${fill}" fill-rule="evenodd" d="${d}"/>`).join('');
  return Buffer.from(
...
await render(markSvg(1024, PURPLE, 0.62, paths), 1024, 'icon.png', '#FFFFFF');
await render(markSvg(512, PURPLE, 0.62, paths), 512, 'icon-512.png', '#FFFFFF');
await render(markSvg(1024, PURPLE, 0.44, paths), 1024, 'adaptive-icon.png', null);
await render(markSvg(1024, PURPLE, 0.68, paths), 1024, 'splash-icon.png', null);
await render(markSvg(96, '#FFFFFF', 0.78, paths), 96, 'notification-icon.png', null);
await render(markSvg(512, PURPLE, 0.82, paths), 512, 'logo.png', null);

await writeFile(
  path.join(OUT, 'README.md'),
  `# WeldAgent App Assets

Generated from the platform's WeldAgent mark
(\`apps/web/platform/public/assets/images/weldagent/icon.svg\`) so the mobile icon
...
Brand color (Android adaptive background + notification tint): \`#8d65ef\`.
`,
);
