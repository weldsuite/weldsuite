    ...
    const init = async () => {
      ...
      try {
        await notifUtils.createNotificationChannels([
          { id: 'meetings', name: 'Meetings', description: 'Reminders, invitations, and meetings starting soon' },
        ]);
        ...
        const cleanup = notifUtils.setupNotificationListeners(
          ...
          (response: any) => {
            ...
            // Meeting events from the backend payload:
            //   { type: 'meeting.invited' | 'meeting.starting_soon' | 'meeting.started', meetingId, joinCode? }
            if (data?.joinCode) {
