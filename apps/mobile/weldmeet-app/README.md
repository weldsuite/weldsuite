...
- `WeldMeet`, display name (e.g. `WeldFoo`)
- `weldmeet-app`, workspace + folder name (e.g. `weldfoo-app`)
- `weldmeet`, lowercase one-word id, used for scheme, bundle suffix, notification `APP_CODE` (e.g. `weldfoo`)
- `Video meetings on the go`, tagline shown on the login screen
- `com.weldsuite.weldmeet`, iOS bundle id / Android package (e.g. `com.weldsuite.weldfoo`)
...
- `services/app-api.ts`, unified app-api client (`@weldsuite/app-api-client` domains + WeldMeet wrapper over `/api/meetings` + `/api/meeting-sessions`)
