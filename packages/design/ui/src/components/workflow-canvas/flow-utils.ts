...
/**
 * Read view over a flow node's `data`.
 *
 * The layout pass walks nodes of every kind generically and only ever inspects
 * these two fields, so it doesn't need to discriminate on node type. Reading
 * through this instead of the specific *NodeData interfaces keeps the layout
 * code from having to narrow at every access.
 */
...
export function getConditionBranchIds(step: { id: string; config?: Record<string, unknown> }): string[] {
  const branches = step.config?.branches;
  if (branches && Array.isArray(branches)) {
    return (branches as ConditionBranchConfig[]).map((b) => `${step.id}_branch_${b.value}`);
  }
...
function getNodeWidth(node: Node): number {
  ...
  const data = node.data as FlowNodeDataView;
...
function getSubtreeNodes(branchNodeId: string, allNodes: Node[]): Node[] {
  ...
  const directChildren = allNodes.filter((n) => {
    const data = n.data as FlowNodeDataView;
  ...
  for (const child of directChildren) {
    ...
    const data = child.data as FlowNodeDataView;
    const stepType = data?.actionType || data?.step?.type || '';
...
function getConditionDepth(conditionNodeId: string, allNodes: Node[]): number {
  ...
  const data = conditionNode.data as FlowNodeDataView;
  ...
  while (parentBranchId) {
    ...
    const parentData = parentCondition.data as FlowNodeDataView;
...
function resolveCollisions(nodes: Node[]): void {
  ...
  for (let iteration = 0; iteration < MAX_ITERATIONS; iteration++) {
    ...
    for (const condNode of conditionNodes) {
      const condData = condNode.data as FlowNodeDataView;
...
function getBranchChildrenHeight(branchChildrenMap: Map<string, WorkflowStep[]>, branchId: string): number {
...
export function getTotalNodeHeight(stepType: string, subAgentCount?: number, branchChildrenMap?: Map<string, WorkflowStep[]>, stepId?: string): number {
  if (stepType === 'condition') {
...
export function workflowToFlow(
  ...
  const triggerNode: Node<TriggerNodeData> = {
    ...
    data: {
      ...
      entityEvent: (() => {
        ...
        // Older saved triggers carry entityType/eventType at the top level;
        // newer ones nest them under `config`.
        const t = trigger as TriggerConfig & { entityType?: string; eventType?: string };
        const entityType = t.entityType || (t.config?.entityType as string | undefined);
        const eventType = t.eventType || (t.config?.eventType as string | undefined);
  ...
  steps.forEach((step) => {
    if (!step.parentBranchId) {
      ...
      const subAgentCount = step.type === 'ai_agent' ? ((step.config?.subAgentIds as string[] | undefined)?.length || 0) : 0;
  ...
  steps.forEach((step, index) => {
    ...
    if (step.parentBranchId && !step.position) {
      const parentBranchId = step.parentBranchId as string;
      ...
      if (branchNode) {
        ...
        if (siblings.length > 0) {
          ...
          const lastSiblingData = lastSibling.data as FlowNodeDataView;
    ...
    const isLastNode = !step.parentBranchId && isLastNonBranchStep;
    ...
    if (step.type === 'condition') {
      ...
      const conditionNode: Node<ConditionNodeData> = {
        ...
        data: {
          ...
          condition: (step.config.expression as string | undefined),
          thenStepId: (step.config.thenAction as string | undefined),
          elseStepId: (step.config.elseAction as string | undefined),
      ...
      const configBranches = step.config?.branches as ConditionBranchConfig[] | undefined;
      if (configBranches && Array.isArray(configBranches)) {
        ...
        const branchWidths = configBranches.map((branch) => {
          ...
          const maxChildWidth = children.reduce((max, child) => Math.max(max, getStepWidth(child.type)), NODE_WIDTH);
        ...
        configBranches.forEach((branch, branchIdx) => {
          const branchNodeId = `${step.id}_branch_${branch.value}`;
          ...
          const branchNode: Node<ConditionBranchNodeData> = {
            ...
            data: {
              ...
              conditionLabel: (step.config.field as string | undefined) || '',
        ...
        const ifMaxWidth = ifChildren.reduce((max, child) => Math.max(max, getStepWidth(child.type)), NODE_WIDTH);
        const ifNotMaxWidth = ifNotChildren.reduce((max, child) => Math.max(max, getStepWidth(child.type)), NODE_WIDTH);
        ...
        const ifBranchNode: Node<ConditionBranchNodeData> = {
          ...
          data: {
            branchType: 'if',
            label: 'If true',
            conditionLabel: (step.config.expression as string | undefined) || 'Condition met',
            parentConditionId: step.id,
            parentConditionStepIndex: index,
      ...
      const finalIsLastNode = step.parentBranchId ? isLastBranchChild : isLastNode;
      ...
      if (step.type === 'ai_agent') {
        const subAgentIds: string[] = (step.config?.subAgentIds as string[] | undefined) || [];
        const subAgentNames: Record<string, string> = (step.config?.subAgentNames as Record<string, string> | undefined) || {};
...
export function autoLayoutNodes(
  ...
  const getTotalNodeHeightFromNode = (node: Node): number => {
    const data = node.data as FlowNodeDataView;
    const stepType = data?.actionType || data?.step?.type || '';
    const subAgentCount = stepType === 'ai_agent' ? ((data?.step?.config?.subAgentIds as string[] | undefined)?.length || 0) : 0;
