...
export const ColorPickerFormat = ({ className, ...props }: ColorPickerFormatProps) => {
  ...
  // Editable hex — local draft state so the user can type arbitrary
  // intermediate values without the parent re-rendering and stomping
  // the input. When the draft is a valid hex we push it up to the
  // ColorPicker context.
  const [hexDraft, setHexDraft] = useState<string>(color.hex());
  useEffect(() => {
  ...
  if (mode === 'hex') {
    return (
      ...
      >
        <Input
          className="h-8 rounded-r-none bg-secondary px-2 text-xs font-mono shadow-none"
          type="text"
          spellCheck={false}
          value={hexDraft}
          onChange={(e) => {
            ...
            // Only commit when we have a full 6-digit hex.
