...
export function SlideshowBlock({
  ...
  return (
    <div className="relative overflow-hidden w-full bg-gray-900" style={{ minHeight: `${minHeight}px` }}>
      {/* Slides Container */}
      ...
      {(showDots || showPlayPause) && slides.length > 1 && (
        <div className="absolute bottom-0 left-0 right-0 z-20">
          ...
          <div className={`relative flex items-center px-6 md:px-8 py-4 ${
            ...
            {/* Play/Pause Button - Positioned Absolutely on Right */}
            {showPlayPause && autoplay && !isEditing && (
              <button
                onClick={() => setIsPaused(!isPaused)}
                className="absolute right-6 md:right-8 w-10 h-10 flex items-center justify-center bg-white/10 hover:bg-white/20 text-white rounded-full transition-all hover:scale-110 active:scale-95"
